...
Speed Reader is a modern web application that helps users improve reading speed and comprehension through various reading techniques. The architecture follows a clean layered approach with clear separation of concerns, type-safe configuration management, structured logging, and distributed rate limiting.

## Architectural Pattern

### Pattern Classification
**Primary Pattern**: Layered Architecture (Service-Repository Pattern)
**Secondary Patterns**:
- Service Layer Pattern (business logic separation)
- Repository Pattern (database abstraction via Drizzle ORM)
- Strategy Pattern (reading mode implementations)
- Factory Pattern (Redis client creation)

### Design Philosophy
- **Separation of Concerns**: Clear boundaries between API, service, and data layers
- **Type Safety**: End-to-end TypeScript with runtime validation
- **Configuration Management**: Centralized environment validation with fail-fast behavior
- **Structured Logging**: JSON logging with request context throughout application
- **Resilient Infrastructure**: Graceful degradation (Redis to in-memory fallback)
- **Rate Limiting**: Distributed rate limiting with configurable thresholds
...
**Responsibility**: Centralized configuration validation and type-safe exports
**Key Functions**:
- Validate all environment variables at application startup
- Provide type-safe exports for configuration access
- Fail fast with descriptive errors for misconfiguration
- Support required and optional variables with defaults
...
export const env = validateEnv(); // Throws on invalid config
```
...
### 2. Logging & Monitoring Layer

#### 2.1 Structured Logger
**Location**: `src/lib/logger.ts`
**Responsibility**: Centralized structured logging with JSON output
**Key Functions**:
- JSON-formatted log output for parsing and analysis
- Request context tracking (userId, sessionId, endpoint, method)
...
// General logging
logger.info('Session completed', { userId, sessionId, wpm: 350 });
logger.warn('Rate limit approaching', { userId, remaining: 2 });
logger.error('Database connection failed', { service: 'db' }, error);
```

**Log Level Selection**:
- Development: DEBUG and above
- Production: INFO and above
- Configured via `NODE_ENV` environment variable

#### 2.2 Request Context
**Helper Function**: `getRequestContext(request: Request)`
**Purpose**: Extract structured context from Next.js requests
**Extracted Data**:
- HTTP method
- Endpoint path
- User agent
...
**Responsibility**: Distributed rate limiting for AI operations
...
const RATE_LIMITS = {
  AI_GENERATION: {
    PER_SESSION: 5,      // Max 5 generations per session
    PER_DAY: 20,         // Max 20 generations per day
    COOLDOWN_MS: 60000,  // 60 second cooldown between requests
  },
};
```

**Operations**:
- `checkAIGenerationRateLimit(userId)` - Check if request allowed
- `recordAIGeneration(userId)` - Record usage
- `getRateLimitStatus(userId)` - Get current status
- `resetUserRateLimits(userId)` - Admin reset function
...
**Responsibility**: Abstract Redis operations with in-memory fallback
...
**Implementations**:
1. **InMemoryRedis**: Development fallback, single-instance state
2. **ActualRedis** (future): Production Redis client
...
- Map-based storage with expiration tracking
